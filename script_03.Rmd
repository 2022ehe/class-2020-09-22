...
# There are two rows for Gov 50, one for the morning section and one for the 
# noon section. By grouping by id, and summarizing by undergrad enrollment, we
# can combine the enrollment for courses that have the same course id.
```
...
```{r sc1}
sep_new <- read_excel("new_data/class_enrollment_summary_by_term_9-22-2020.xlsx", 
             skip = 3) %>% 
          clean_names() %>%
          filter(! is.na(course_title)) %>% 
          select(-grad, -non_degree, -x_reg, -vus, -employee, 
...
- go through and decide which data is relevant to the problem we want to solve 
(e.g. undergraduate enrollment vs. graduate)
- classes that dropped vs. enrollment dropping
- taking into account people who are taking a leave of absence
- make assumptions so that we don't have an infinite amount of data


After we have a model, Temperance suggests we should:
- recognize limitations of our model, 
...
ggplot(sep_diff, aes(x = title, y = u_grad_diff)) + 
  geom_col()
